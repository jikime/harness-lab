...
- 실행 하네스 파일은 현재 작업 중인 프로젝트 루트에 만든다. 기본 생성 위치는 현재 프로젝트의 `.claude/agents`, `.claude/skills`, `CLAUDE.md`, `artifacts/`이다.
...
- 실행 하네스 생성 시에는 현재 프로젝트의 `.claude/agents`, `.claude/skills`, `CLAUDE.md` 포인터를 기준으로 구성한다.
...
- Orchestrator 역할을 하는 Skill은 폴더명이 반드시 `-orchestrator`로 끝나야 한다. 예: `.claude/skills/trip-planning-orchestrator/SKILL.md`. frontmatter `name`을 쓴다면 폴더명과 같은 값을 사용한다.
...
- Agent 정의에는 역할에 맞는 `tools` frontmatter를 명시한다. 조사와 검토 역할은 읽기 도구 중심으로 좁히고, 구현 역할에만 필요한 쓰기와 실행 도구를 준다. `model`은 기본 Agent frontmatter에 넣지 않는다. 직접 Subagent는 기본 모델 정책을 따르게 두고, Agent Team은 팀 생성 지시와 teammate 모델 설정을 실행 기록에 남긴다.
- Agent 파일명과 frontmatter `name`, Orchestrator에서 부르는 Agent 이름은 서로 맞아야 한다. 이름이 어긋나면 실행 전에 수정 후보로 표시한다.
...
- 부분 재실행에서는 바뀐 산출물을 읽는 뒤 단계 파일을 찾아 `stale`로 표시하고, 필요한 단계만 다시 실행하거나 사용자에게 재검토 범위를 확인한다.
- 사용자가 HTML, 대시보드, 시각 리포트 같은 산출물을 요구하면 산출물 계약에 `.html` 파일을 포함하고, 카드, 표, 차트, 체크리스트, 검증 기준을 함께 설계한다.
- 사용자가 TailwindCSS나 Chart.js 같은 프론트엔드 라이브러리를 요구하면 단일 HTML 산출물에서는 CDN 방식을 허용하되, 인터넷 연결 필요성과 운영 배포용 빌드 방식의 차이를 짧게 안내한다.
...
| 운영/유지보수 | drift, 중복, 누락 테스트, 오래된 포인터를 점검하고 개선 청사진을 제시한다 |

기존 확장에서는 오케스트레이터를 무조건 새로 만들지 않는다. 기존 Orchestrator가 있으면 팀 구성, Task 계약, 산출물 경로, description 후속 키워드를 갱신하는 방식을 먼저 검토한다.
...
Agent Team으로 간주하려면 Agent 정의 파일만 있으면 안 된다. Orchestrator Skill 안에 팀 생성, 작업 등록, 메시지 규칙, 파일 산출물, 최종 통합, 팀 정리 절차가 있어야 한다. 구체적인 계약은 `references/agent-team-design.md`를 따른다.
...
산출물 구조는 `references/orchestrator-artifacts-template.md`를 따른다. 작은 하네스라도 입력, 중간 결과, 검토, 최종 결과, 개선 기록이 어디에 남는지 보여야 한다.
...
- `references/agent-team-design.md`: Agent/Skill/Orchestrator 구분, 실행 모드, Agent Team 계약, 팀 패턴, Agent 파일 구조가 필요할 때
- `references/orchestrator-artifacts-template.md`: 청사진, Agent, Skill, Orchestrator, `CLAUDE.md`, `artifacts/` 산출물 템플릿이 필요할 때
...
- Orchestrator description에는 초기 실행 키워드뿐 아니라 재실행, 부분 수정, 업데이트, 보완, 이전 결과 기반 개선 같은 후속 작업 키워드를 포함한다.
...
- 기존 하네스를 수정하면 `CLAUDE.md`나 별도 개선 기록에 날짜, 변경 내용, 대상, 사유를 남긴다.
