...
무엇을 번들링할지는 추측하지 말고 실행 트랜스크립트에서 신호를 본다. 아래가 보이면 `scripts/` 후보다.

| 신호 | 조치 |
| --- | --- |
| 테스트 3회 중 3회에서 같은 헬퍼 스크립트를 새로 생성 | 그 스크립트를 `scripts/`에 번들 |
| 매번 같은 의존성 설치를 반복 | 설치 단계를 Skill 본문에 명시 |
| 같은 다단계 접근을 매번 반복 | 표준 절차로 본문에 기술 |
| 비슷한 에러 뒤 같은 회피책을 반복 | 알려진 문제와 해결법을 본문에 기록 |

번들링한 스크립트는 반드시 실행 테스트를 거친다.
