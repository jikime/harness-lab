...
필드명은 `text`, `passed`, `evidence`, `summary.{passed,failed,total,pass_rate}`를 정확히 쓴다(`name`/`met`/`details` 같은 변형 금지). 여러 Agent나 스크립트가 채점 결과를 읽고 합칠 때 필드명이 흔들리면 집계가 깨진다.
...
하네스가 코드를 만들면 위 "코드 연결" 행이 가장 자주 깨지는 지점이다. 두 컴포넌트가 각각은 "올바른데" 연결부의 계약이 어긋나는 **경계면 불일치**가 런타임 오류의 주원인이다. 아래 구체 패턴을 검토 대상으로 명시한다(비코드 하네스면 이 절은 건너뛴다).

| 경계면 | 흔한 불일치 | 놓치는 이유 |
| --- | --- | --- |
| 응답 shape ↔ 소비자 타입 | API가 `{ items: [...] }` 반환, 소비자는 배열 기대 | 각각은 검증되지만 교차 대조를 안 함 |
| 필드 표기 | `thumbnailUrl`(camelCase) ↔ `thumbnail_url`(snake_case) | 제네릭 캐스팅이 컴파일러 검출을 우회 |
| 파일 경로 ↔ 링크 | 페이지는 `/dashboard/create`인데 링크는 `/create` | 파일 구조와 href를 함께 안 봄 |
| 상태 전이맵 ↔ 실제 update | 맵에 정의된 전이가 코드에 없음(죽은 전이), 또는 코드 전이가 맵에 없음(무단 전이) | 맵 존재만 확인하고 update 코드를 추적 안 함 |
| 즉시 응답 ↔ 비동기 결과 | API가 즉시 `{ status }` 반환, 소비자가 최종 결과 필드 접근 | 동기/비동기 구분 없이 타입만 확인 |
| 엔드포인트 ↔ 호출부 | API는 있으나 대응 호출/훅이 없어 실제로 안 불림 | 엔드포인트와 호출부를 1:1 매핑 안 함 |

핵심 통찰 두 가지:

- **빌드 통과 ≠ 런타임 정상.** 타입 캐스팅·`any`·제네릭이 끼면 컴파일은 성공해도 런타임에 shape이 어긋난다. "빌드가 된다"를 검증 통과로 쓰지 않는다.
- **존재 확인 ≠ 연결 확인.** "API가 있는가"와 "그 응답이 호출부 기대와 맞는가"는 다른 검증이다. 후자를 하려면 생산자와 소비자 코드를 **동시에 열어** 대조한다.

이 패턴은 결함 주입 회귀 세트(위 "결함 주입과 회귀 세트")에도 그대로 심는다 — 응답 shape 변조, 깨진 라우트, 누락 상태 전이를 일부러 넣어 검토가 잡는지 확인한다.
...
handoff는 한 실행 *안*에서 다음 단계로 넘기는 인계고, 개선 기록·ADR은 사람이 읽는 회고다. 그 사이에 빠지기 쉬운 것이 **다음 실행의 Agent가 지난 실행의 교훈을 자동으로 읽는** 경로다. 이게 없으면 같은 실수가 실행마다 반복되고, 진화가 "사람이 파일을 손으로 고칠 때만" 일어난다.

경량 장치: 하네스별 `artifacts/lessons.md`(또는 `.claude/skills/{harness}-orchestrator/lessons.md`)에 반복 실패와 그 회피책을 누적하고, **Orchestrator 실행 흐름의 시작 단계에서 이 파일을 읽어** 관련 Agent의 Task 설명이나 prompt에 해당 교훈을 주입한다.

| 항목 | 내용 |
| --- | --- |
| 무엇을 적나 | 반복된 실패 유형, 그때의 입력 특징, 검증이 놓친 지점, 채택한 회피책 |
| 언제 적나 | 개선 기록을 쓸 때 그중 "다음 실행이 미리 알아야 할 것"만 추린다 |
| 언제 읽나 | Orchestrator가 초기화 단계에서 읽어, 관련 Task/Agent에 주입한다 |
| 무엇과 다른가 | handoff=실행 내 인계, 개선 기록=사람용 회고, lessons=다음 실행 Agent가 읽는 운영 지식 |

lessons.md도 무한히 커지면 노이즈가 되므로, 하네스 구조에 반영해 상시 지켜지게 된 항목(예: 이미 Skill·검증에 녹인 규칙)은 주기적으로 걷어내 개선 기록으로만 남긴다. 저위험·일회성 하네스에는 이 장치를 두지 않는다 — 반복 실행되는 하네스에서만 이득이 있다.
