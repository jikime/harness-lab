...
이 프로젝트의 핵심 스킬은 `skills/harness-lab`입니다. 이 스킬은 사용자의 업무를 바로 자동화해주는 도구라기보다, 업무를 하네스 구조로 바꾸는 연습을 도와주는 안내자입니다.
...
이 흐름을 쓰면 사용자가 내부 파일 구조를 몰라도 됩니다. 먼저 설계안을 읽어보고, 마음에 들 때만 실행 가능한 하네스 구성을 승인하면 됩니다.
...
이렇게 요청하면 `harness-lab`이 필요한 Claude Code 하네스 구조를 대신 선택합니다. 사용자가 `.claude/agents`나 `.claude/skills` 같은 용어를 미리 알 필요는 없습니다.
...
`harness-lab`이 실행 하네스를 만들 때 생성하거나 안내하는 파일은 모두 역할이 다릅니다. 파일명을 외우기보다 "팀원 카드, 작업 매뉴얼, 전체 진행표, 안내판"으로 이해하면 쉽습니다.
...
| `.claude/skills/{orchestrator}/SKILL.md` | 전체 진행표 | 여러 Agent와 Skill을 어떤 순서로 실행하고, 중간 산출물을 어떻게 이어받을지 정합니다. |
| `CLAUDE.md` | 프로젝트 안내판 | 이 프로젝트에서 만든 하네스가 어디에 있고, 어떤 흐름으로 써야 하는지 Claude Code가 참고할 포인터를 남깁니다. |
| `artifacts/` 또는 중간 산출물 파일 | 작업 기록지 | 실행 중 조사 노트, 초안, 검토 결과, 최종 결과처럼 다음 단계가 이어받을 자료를 남깁니다. |
| 개선 기록 | 다음 버전 메모 | 실제로 써본 뒤 부족했던 점과 다음에 고칠 규칙을 기록합니다. |

처음에는 Orchestrator Skill만 기억해도 충분합니다. Orchestrator가 전체 입구이고, Agent와 개별 Skill은 그 흐름 안에서 필요한 때에 쓰입니다.
