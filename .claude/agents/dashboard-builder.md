...
- `02-itinerary-draft.md`·예산이 안 바뀌었는데 대시보드 버전 번호(v1, v2...)를 임의로 올리지 않는다. 버전 번호는 그 itinerary 버전을 가리키는 표기이므로, 소개 섹션 추가처럼 데이터가 그대로인 변경은 같은 버전 번호를 유지하고 "무엇을 추가했는지"만 푸터에 짧게 남긴다.
