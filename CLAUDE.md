...
| 2026-09-03 | `02-itinerary-draft.md` 이력 슬림화(9,282줄 → 약 2,700줄)와 "이력 누적 금지" 전환: 회차별 수정 이력 챕터·재검토 스코프·폐기 대안 서술을 draft에서 삭제하고 git 커밋 + draft 9장 「개정 이력 요약」 한 줄 표에 위임. planner는 앞으로 draft에 개정 챕터를 쌓지 않음(출력·작업 방식 8에 명문화) | itinerary-planner, family-trip-prep-orchestrator, (02-itinerary-draft.md) | `02-itinerary-current.md` 스냅샷을 뒀는데도(2026-09-02) draft가 v29~v32에서 계속 커졌고, 사용자가 "폐기된 건 지워버려 — 데이터가 너무 많으니까 못 찾는 거 아냐"라고 지시. AskUserQuestion으로 "draft를 현행만 남기고 슬림화(current.md 병존)"를 선택받아 실행. git이 이미 전체 이력을 보존하므로 파일 내 이력 보존은 중복이었고, 그 중복이 잔존 오류의 은신처였음 |
